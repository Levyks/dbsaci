...
2. **Translate.** The Oracle SQL / PL-SQL text is parsed and rewritten to
   PostgreSQL SQL. Binds are sent as PostgreSQL parameters. Oracle scalar
   functions map to [`orafce`](https://github.com/orafce/orafce).
3. **Execute.** One backend round trip to PostgreSQL. The result is streamed —
   pgSaci does not buffer the whole result set.
...
pgSaci picks each of those from the capabilities the client sends in the TTC
...
The main suite is the **golden corpus**: a real PostgreSQL/`orafce` container and
a real pgSaci proxy, every case driven over TNS, checking Oracle-correct values,
row counts, and error text. CI runs it against PostgreSQL 18, 16, and 13.
