...
| Area | Status | Notes |
| --- | --- | --- |
| **Connection / authentication** | Supported | Full TNS/TTC handshake + auth for `python-oracledb` thin, the Oracle JDBC thin driver, ODP.NET managed, `oracle-rs`, and the OCI thick client (Instant Client, via `python-oracledb` thick mode) — each verified end to end against both the 19c and 11g personas. 11g clients that only speak O5LOGON get the MD5 verifier; JDBC and ODP.NET 11g use 12c PBKDF2. One Oracle session ↔ one dedicated PostgreSQL connection. Credential model: a pre-declared `pg_user → pg_password` list with an optional fallback; the Oracle username is matched case-insensitively and an unmatched user with no fallback is `ORA-01017`. |
| **SQL SELECT / DML** | Supported subset | Joins, CTEs (incl. recursive), windows/analytics, `ROWNUM`, `FETCH FIRST n ROWS ONLY`, legacy `(+)`, `CONNECT BY` (`NOCYCLE`, `CONNECT_BY_ISCYCLE/ISLEAF/ROOT`, `SYS_CONNECT_BY_PATH`), `MERGE` (incl. matched `DELETE`), `INSERT ALL`/`INSERT FIRST`, `PIVOT`/`UNPIVOT`, `REGEXP_*`, sequences, and common `orafce` functions. |
| **Data dictionary** | Supported subset | `USER_*` / `ALL_*` catalog views for tables, columns, objects, constraints, indexes, sequences, and comments; `V$VERSION`; `NLS_SESSION_PARAMETERS`; `DUAL`. |
...
| **Errors** | Partial | ~40 PostgreSQL SQLSTATEs → Oracle error numbers (deadlock, serialization failure, unique / FK / not-null / check violation, `NOWAIT` resource-busy, statement cancellation, connection loss, invalid number/date, subquery-returns-many, and more). The PostgreSQL statement character position is carried into the TTC `error_pos` field. |
